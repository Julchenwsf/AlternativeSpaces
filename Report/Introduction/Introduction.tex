...
In the following section the main focus will be on the risk management of the project. The whole project will be analysed and every risk will be grouped by the following criteria:
\begin{enumerate}
\item Group communication
\item Issues with the customer
\item Risk in relation to planning
\end{enumerate}
%TODO REPHRASE
The ID of risks displays which criteria belong to the risk. By using the probability of the risks and their consequence level an evaluation of risks to the project can be made. The following table shows the meaning of consequence levels.
...
In order to evaluate the probability of a risk happening, the group members' experience is important. Each member of the group ranks the risks according to what they think the chance of a risk happening is (high or low). A clear majority determines the result, and an unclear majority or tie results in a medium probability.


When we have determined both these values for the risks, we can calculate their risk levels. The risk level is found using the followin formula.
$$
\text{Risk level} = \text{Consequence level} \cdot \text{probability}
$$
At this the level high is counted as “1”, medium as “0.5” and low as “0”. With this formula the results will be four different values (1, 0.5, 0.25, 0). To decide for which risk level a further elaboration is necessary and useful the following matrix was established.\\
...
\begin{tabular}{cc|c|c|c| }
  ...
  \multicolumn{1}{|c}{\multirow{3}{*}{Prob.}} & \multicolumn{1}{|c|}{High} & \cellcolor{red!30} High & \cellcolor{red!30} High-Medium & \cellcolor{green!15} Low \\ \cline{2-5}
  \multicolumn{1}{|c}{} & \multicolumn{1}{|c|}{Medium} & \cellcolor{red!30} High-Medium & \cellcolor{yellow!30} Medium-High & \cellcolor{green!15} Low \\ \cline{2-5}
  \multicolumn{1}{|c}{} & \multicolumn{1}{|c|}{Low} & \cellcolor{green!15} Low & \cellcolor{green!15} Low & \cellcolor{green!15} Low \\ \cline{2-5}
  \hline
\end{tabular}
\captionof{table}{Different risk levels.} 
\end{minipage}