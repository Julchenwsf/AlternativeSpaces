...
%TODO REWRITE
After analysing these two different models it was clear that both have their strengths and weakness. The vision of the customer what they expected signalised that this project is a research project. The requirements were not described in detail or fixed. There were many freedoms and making choices will be a focus in this project. For this reason it was necessary to choose a model where changes in the requirements are easy to handle. Another thing that was important for the choice was the goal of the project. The product should be used by youth so the usability was very important. The customer arranged a meeting with potential users in Oslo in the start of October which also led to the preference for an iterative approach as it would allow us to have a concrete, if somewhat limited, product to showcase at this meeting. The youth and the customer should be included in the project as early as possible to give feedback. So we have needed a model that shows very early parts of the final product and also has much flexibility for changings of requirements. For this reason the waterfall model was not suited, because the interaction with the customer is very late and changes in requirements are very difficult to implement. On the contrary the scrum model was perfect for both needs. On that score the choice for our project was scrum.